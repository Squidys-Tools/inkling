...
import { normalizeNoteBody, noteBodyForPreview } from "../lib/notes";
...
export function RichNoteEditor({ body, title, onSave }: RichNoteEditorProps) {
  ...
  const save = async () => {
    const nextBody = normalizeNoteBody(editor.getMarkdown());
    if (!nextBody) {
