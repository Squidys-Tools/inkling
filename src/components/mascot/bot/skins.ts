...
const inklingSplash = normalize(
  ANGLES.map((a) => 1 + 0.09 * Math.cos(5 * a + 0.8) + 0.05 * Math.cos(2 * a + 2.0)),
  1.06
)
...
export type ColorId =
  ...
  | 'rose'
  | 'gris'
  // INKLING EXTENSION: near-black body from docs/design.md character tokens.
  | 'inkling'
...
export const COLORS: BotColor[] = [
  ...
  { id: 'gris', hex: '#a3a3a3' },
  { id: 'creme', hex: '#f1efe9' },
  // INKLING EXTENSION: character body per docs/design.md (ink primary).
  { id: 'inkling', hex: '#1a1a1a' }
]
