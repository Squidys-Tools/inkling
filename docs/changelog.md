...
- The local extension receiver now accepts the Tauri development origin and answers Private Network Access preflights, so the in-app connection test and paired browser saves can reach the running companion (`src-tauri/src/capture_server.rs`, `src/App.tsx`)
