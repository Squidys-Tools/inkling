...
- Sidebar mascot now commutes into the search field on focus (attentive, curious with a bob while typing) through one shared engine while the sidebar slot collapses so the wordmark slides over; the focused field shows only the mascot's eyes and gains a state-driven wavy outline (calm ripple idle, stronger while typing or processing) and the old static field equalizer is retired (`src/App.tsx`, `src/App.css`)
