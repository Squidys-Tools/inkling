...
- The mascot wanders the library on its own: 90 to 180 seconds after startup, and again after a long cooldown, it leaves the sidebar slot, walks a weighted random route of glances, short hops, crossings and margin drifts for 15 to 35 seconds, wears a random expression at each stop, and drifts home — watching what the user does without being cancelled by it, holding still under overlays, going home sad when a capture fails, and never leaving home under reduced motion. It stands only in measured free space (never over a card, control, toast or chrome), takes no pointer or keyboard input, and sits under every modal, with a faint silhouette left in the slot while it is away, plus a dev board at `?mascot-roam` for forcing outings, speeding the clock, reseeding, and stepping decision by decision (SQU-58) (`src/components/mascot/roam.ts`, `src/components/mascot/safeSpots.ts`, `src/components/mascot/roamStore.ts`)
