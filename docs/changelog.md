...
- Five sidebar aliveness variations on a dev-only board at `?mascot-alive` (slow sway and jelly as new looping engine states, a social state-cycler, a cursor-tracking watcher, and a shape-drift morph with a new splash-b skin), all reusing the vendored engine without touching measured states (`src/components/mascot/AliveBoard.tsx`)
