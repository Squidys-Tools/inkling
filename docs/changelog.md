...
- Sidebar mascot now commutes into the search field on focus (attentive, curious with a bob while typing) through one shared engine while the sidebar slot collapses so the wordmark slides over; the focused field gains a state-driven wavy outline (calm ripple idle, livelier while typing or processing) and the old static field equalizer is retired (`src/App.tsx`, `src/App.css`)
