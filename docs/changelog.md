...
- Sidebar mascot now commutes into the search field on focus (attentive, curious with a bob while typing) through one shared engine, leaving a wavy mini placeholder behind; the old hide-on-focus logo behavior and the static field equalizer are retired (`src/App.tsx`, `src/App.css`)
