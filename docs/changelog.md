...
- Sidebar mascot now commutes into the search field on focus (attentive, curious with a bob while typing, wavy field outline) through one shared engine, leaving a breathing sleepy splash behind so the sidebar slot is never empty; the old hide-on-focus logo behavior and the static field equalizer are retired (`src/App.tsx`, `src/App.css`)
