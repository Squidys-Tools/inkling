...
mod windows_impl {
    ...
    impl WindowsMediaOcr {
        ...
        fn extract_text_inner(
            &self,
            image_bytes: &[u8],
        ) -> std::result::Result<Option<String>, OcrError> {
            unsafe { RoInitialize(RO_INIT_MULTITHREADED) }.map_err(|e| OcrError {
                code: "runtime-init",
                message: e.to_string(),
            })?;
            ...
            let result = self
                .runtime
                .block_on(async { extract_ocr_text(image_bytes).await });
    ...
    async fn extract_ocr_text(image_bytes: &[u8]) -> std::result::Result<Option<String>, OcrError> {
        let stream = InMemoryRandomAccessStream::new().map_err(|e| OcrError {
            code: "stream-create",
            message: e.to_string(),
        })?;

        {
            let writer = DataWriter::CreateDataWriter(&stream).map_err(|e| OcrError {
                code: "writer-create",
                message: e.to_string(),
            })?;
            writer.WriteBytes(image_bytes).map_err(|e| OcrError {
                code: "stream-write",
                message: e.to_string(),
            })?;
            let _ = writer
        ...
        let size = stream.Size().map_err(|e| OcrError {
            code: "stream-size",
            message: e.to_string(),
        })?;
        if size == 0 {
        ...
        let bitmap = decoder
            ...
            .map_err(|e| OcrError {
                code: "software-bitmap-create",
                message: e.to_string(),
            })?
            .await
            .map_err(|e| OcrError {
                code: "software-bitmap",
                message: e.to_string(),
            })?;
    ...
    fn ensure_bitmap_format(
        bitmap: SoftwareBitmap,
    ) -> std::result::Result<SoftwareBitmap, OcrError> {
        let current_format = bitmap.BitmapPixelFormat().map_err(|e| OcrError {
            code: "pixel-format",
            message: e.to_string(),
        })?;
        ...
        let converted =
            SoftwareBitmap::Convert(&bitmap, BitmapPixelFormat::Bgra8).map_err(|e| OcrError {
                code: "convert-format",
                message: e.to_string(),
            })?;
