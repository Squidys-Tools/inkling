...
- **Audit the whole route.** Install a `requestAnimationFrame` audit that compares `.mascot-roamer` with each `.library-card` clipped to `.library-scroll`, plus `.capture-bar`, `.library-toolbar`, `.search-field`, `.add-button`, `.nav-item`, and `[data-sonner-toast]`. Wait until `.brand-mark` loses `is-roaming`. Expect zero overlaps, zero out-of-viewport frames, several state changes, and a different stop count on another seed.
