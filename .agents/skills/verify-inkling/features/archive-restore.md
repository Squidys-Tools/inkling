...
Forgetting an item removes it from the active library. The desktop app archives the row and offers Undo in a persistent toast. Settings then lists archived items for recovery or permanent deletion. The web preview keeps both active items and the seeded archive in memory, and its Forget action does not add the forgotten item to the archive.
...
- The preview's Forget action removes only from `items`; it does not append to `archivedItems`. The seeded archive stays at three until recovery or deletion changes it.
