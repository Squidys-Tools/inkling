...
- `archive-in` removes an item with `Forget this item` in the detail overlay or `Forget` in Serendipity.
...
- Open an item and choose `Forget this item`, or choose `Forget` in Serendipity.
...
- **Read the archive.** Click `[aria-label="Settings"]`, wait for `.settings-modal`, and assert `.settings-panel-count` reads `3 items in archive`. Read `.settings-archive-grid .library-card` and expect the IDs `archive-10`, `archive-12`, and `archive-5` as a set. Preview order follows the seed array; native order is `updated_at DESC`. The Archive tab badge tracks the same count.
...
- Preview batch recovery re-inserts selected items with `archived: true` and their `archive-` ids unchanged, unlike single-item Undo which clears the flag. The active grid shows the card, but Serendipity excludes it. Scope archive assertions to `.settings-archive-grid`; a bare `[data-library-item-id="archive-…"]` can match the recovered card behind the modal.
